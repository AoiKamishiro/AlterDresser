...
namespace online.kamishiro.alterdresser.editor.localization
{
    internal static class Localizer
    {
        ...
        internal static Translation[] _translationData = Array.Empty<Translation>();

        internal static int LanguageIndex
        {
            get => EditorPrefs.GetInt("Localizer.LangIdx");
            set
            {
                EditorPrefs.SetInt("Localizer.LangIdx", value);
                Update();
            }
        }
        ...
        private static Translation[] TranslationData
        {
            get
            {
                if (_langList == null || _langList.Length < 1) Load();
        ...
        {
            _translationData = Array.Empty<Translation>();
            AssetDatabase.FindAssets("*", new string[] { AssetDatabase.GUIDToAssetPath("0152b7957868a8d499247d3b14fd59b0") })
                ...
                .ForEach(x =>
                {
                    ...
                    _translationData = _translationData.Append(JsonUtility.FromJson<Translation>(x.text)).ToArray();
                });
